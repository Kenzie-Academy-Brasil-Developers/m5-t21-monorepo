...
describe("UNIT: LoanService.create", () => {
  ...
  test("should return a new loan", async () => {
    ...
    jest
      ...
      .mockResolvedValueOnce(MAX_LOANS_PER_MEMBER - 1);

    // 4. Mockando loan.create
    jest.spyOn(prisma.loan, "create").mockResolvedValueOnce(loanValidData);

    // 5. Mockando book.update
    jest
      .spyOn(prisma.book, "update")
      .mockResolvedValueOnce({ ...bookData, id: loanValidData.bookId });
    ...
    expect(prisma.loan.create).toHaveBeenCalledWith({ data: loanValidData });

    expect(prisma.book.update).toHaveBeenCalledTimes(1);
    expect(prisma.book.update).toHaveBeenCalledWith({
      where: { id: loanValidData.bookId },
